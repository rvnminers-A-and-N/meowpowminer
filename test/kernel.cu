...
__device__ __forceinline__ void progpowprimeLoop(const uint32_t loop,
    uint32_t mix[PROGPOWPRIME_REGS],
    const dag_t *g_dag,
    const uint32_t c_dag[PROGPOWPRIME_CACHE_WORDS],
    const bool hack_false)
{
    ...
    // global load
    offset = __shfl_sync(0xFFFFFFFF, mix[0], loop%PROGPOWPRIME_LANES, PROGPOWPRIME_LANES);
    offset %= PROGPOWPRIME_DAG_ELEMENTS;
    offset = offset * PROGPOWPRIME_LANES + (lane_id ^ loop) % PROGPOWPRIME_LANES;
    data_dag = g_dag[offset];
    // hack to prevent compiler from reordering LD and usage
    if (hack_false) __threadfence_block();
    // cache load 0
    offset = mix[26] % PROGPOWPRIME_CACHE_WORDS;
    data = c_dag[offset];
    mix[0] = (mix[0] ^ data) * 33;
    // random math 0
    data = mix[10] ^ mix[16];
    ...
    // cache load 1
    offset = mix[30] % PROGPOWPRIME_CACHE_WORDS;
    data = c_dag[offset];
    ...
    // cache load 2
    offset = mix[1] % PROGPOWPRIME_CACHE_WORDS;
    data = c_dag[offset];
    ...
    // cache load 3
    offset = mix[19] % PROGPOWPRIME_CACHE_WORDS;
    data = c_dag[offset];
    ...
    mix[7] = (mix[7] ^ data) * 33;
    // cache load 4
    offset = mix[11] % PROGPOWPRIME_CACHE_WORDS;
    data = c_dag[offset];
    ...
    mix[8] = (mix[8] * 33) + data;
    // cache load 5
    offset = mix[21] % PROGPOWPRIME_CACHE_WORDS;
    data = c_dag[offset];
    mix[9] = (mix[9] ^ data) * 33;
    // random math 5
    ...
    // cache load 6
    offset = mix[15] % PROGPOWPRIME_CACHE_WORDS;
    data = c_dag[offset];
    ...
    // cache load 7
    offset = mix[18] % PROGPOWPRIME_CACHE_WORDS;
    data = c_dag[offset];
    ...
    // cache load 8
    offset = mix[3] % PROGPOWPRIME_CACHE_WORDS;
    data = c_dag[offset];
    mix[6] = (mix[6] ^ data) * 33;
    // random math 8
    ...
    // cache load 9
    offset = mix[17] % PROGPOWPRIME_CACHE_WORDS;
    data = c_dag[offset];
    ...
    // cache load 10
    offset = mix[31] % PROGPOWPRIME_CACHE_WORDS;
    data = c_dag[offset];
    mix[2] = (mix[2] * 33) + data;
    // random math 10
    data = mix[11] | mix[17];
    ...
    // cache load 11
    offset = mix[16] % PROGPOWPRIME_CACHE_WORDS;
    data = c_dag[offset];
