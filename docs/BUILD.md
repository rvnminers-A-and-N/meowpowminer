...
cmake -G "Visual Studio 15 2017 Win64" -H. -Bbuild -DETHASHPRIMECL=ON -DETHASHPRIMECUDA=ON -DAPICORE=ON ..
...
```shell
cmake .. -DETHASHPRIMECUDA=ON -DETHASHPRIMECL=OFF
```

* `-DETHASHPRIMECL=ON` - enable OpenCL mining, `ON` by default.
* `-DETHASHPRIMECUDA=ON` - enable CUDA mining, `ON` by default.
